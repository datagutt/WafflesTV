 ...
 *
 * @param $date Date of the object under test
 * @param $from Other end of the interval, current date if unspecified
 * @return int
 */
function age_in_years(\DateTime $date, \DateTime $from = null) {
    return intval(date_diff(
        $date,
        null === $from ? date_create(date('Y-m-d')) : $from,
        true
    )->format('%y'));
}
/*
 * Copyright 2012 Nettsamfunn1.
 */
function makeCSRF(){
...
function validateCSRF(){
	if(isset($_POST['token']) && $_POST['token'] === makeCSRF()){
		unset($_SESSION['csrf']);
		return true;
	}
	die('Invalid CSRF key!');
}